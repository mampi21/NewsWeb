...
export class News extends Component {
  ...
  constructor(props){
    ...
    document.title = `${(this.props.category).charAt(0).toUpperCase() + (this.props.category).slice(1)} - NewsWeb`;
  ...
  async componentDidMount(){
    // let url = `https://newsapi.org/v2/top-headlines?country=${this.props.country}&category=${this.props.category}&apikey=${process.env.REACT_APP_NEWS_API}&pageSize=${this.props.pageSize}`;
    // this.setState({loading:true})
    // let data = await fetch(url);
    // let parsedData = await data.json()
    // console.log(data);
  ...
  render() {
    return (
      <>
        <h1 className="text-center" style={{margin: '35px', marginTop: "90px"}}> NewsWeb -Top {(this.props.category).charAt(0).toUpperCase() + (this.props.category).slice(1)} Headlines</h1>
